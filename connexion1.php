...
if (!empty($_POST['loginn']) && !empty($_POST['pwd'])){
	...
	foreach($u as $t){
		$vide=true;
	if ($t['user_name']==$_POST['loginn'] && $t['user_pass']==$_POST['pwd'] && $t['role']=="Client"){
		...
		header("location:../front/index2.html");
		
		}
		else if ($t['user_name']==$_POST['loginn'] && $t['user_pass']==$_POST['pwd'] && $t['role']=="Admin")
			{
				session_start();
		$_SESSION['l']=$_POST['loginn'];
		$_SESSION['p']=$_POST['pwd'];
...
if ($vide==false) { 
         ...
         echo '<meta http-equiv="refresh" content="0;URL=../front/index.html">'; 
      } 
}	  
 
else { 
      echo "Les variables du formulaire ne sont pas déclarées.<br> Vous devez remplir le formulaire"; 
     ?> <a href="../front/index.html">Retour au formulaire</a>	 <?php 
}  
